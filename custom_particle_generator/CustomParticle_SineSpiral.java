...
public class CustomParticle_SineSpiral {
	public static void SineSpiral() throws IOException {		//这里之所以不需要粒子名称是因为如果需要不同的粒子，base_uv里的值也会相应更改，而需要换行的粒子则更加复杂（第二种粒子）
		Apfloat x = new Apfloat(0);												//这两个变量用于定义文件名，而非真正粒子的目的地
		Apfloat y = new Apfloat(1.0);
		...
		Apfloat FourOverPi = new Apfloat(0);									//常量4/pi（#28赋值）
		final Apfloat aFour = new Apfloat(4.0);									//常量4（Apfloat类型）
		final Apfloat aTwo = new Apfloat(2.0);									//常量2（Apfloat类型）
		FourOverPi = aFour.divide(ApfloatMath.pi(16));							
		int i = 1;			//debug用变量
		int g = 1;
		int h = 1;
		int k = 1;
		...
		for (t.add(Zero); (t.compareTo(Tau) <= 0); t = t.add(step)) {	//等同于 for(t+0; t<=2PI; t+=PI/40), +0是因为直接用t会报错
			...
			BigDecimal bx = new BigDecimal(FourOverPi.multiply(ApfloatMath.sin(aT.multiply(aTwo)).multiply(ApfloatMath.cos(aT))).toString());
																		//真正的desX = (4/pi)*sin(2t)*cos(t), (4/pi)用于调整大小
																		//计算之后转回BigDecimal，写文件时Apfloat会使用科学计数法（其实mc能读，debug方便）
			BigDecimal by = new BigDecimal(FourOverPi.multiply(ApfloatMath.sin(aT.multiply(aTwo)).multiply(ApfloatMath.sin(aT))).toString());
																		//真正的desY = (4/pi)*sin(2t)*sin(t), (4/pi)用于调整大小
			String name = String.format("endRod_SineSpiral_%.2f_%.2f", x.doubleValue(), y.doubleValue());
																		//文件名称：endRod_SineSpiral_desX（两位小数）_desY（两位小数）
			...
			String identifier = String.format("\"identifier\": \"minecraft:endRod_sineSpiral_%.2f_%.2f\",", x.doubleValue(), y.doubleValue());
			...
			if (t.compareTo(Tau.divide(Four)) <= 0) {		//如果是前四分之一（圆的右上角）；等同于 if (t <= PI / 2)
				x = x.add(OneOverTwenty);		//desX += 0.05
				y = y.subtract(OneOverTwenty);	//desY -= 0.05
				System.out.println(i);
				i++;
			} else if (t.compareTo(Tau.divide(Two)) <= 0) {	//如果是前四分之一到一半（圆的右下角）；等同于 if (t <= PI)
				x = x.subtract(OneOverTwenty);	//desX -= 0.05
				y = y.subtract(OneOverTwenty);	//desY -= 0.05
				...
				x = x.subtract(OneOverTwenty);	//desX -= 0.05
				y = y.add(OneOverTwenty);		//desY += 0.05
	...
	public static void main(String[] args) throws IOException {
		CustomParticle_SineSpiral.SineSpiral();	//调用方法，生成80种不同的粒子
	}
}
