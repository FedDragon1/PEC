...
public class CustomParticle_Circle {
	public static void Circle () throws IOException {	//这里之所以不需要粒子名称是因为如果需要不同的粒子，base_uv里的值也会相应更改，而需要换行的粒子则更加复杂（第二种粒子）
		...
		for (t.add(Zero); (t.compareTo(Tau) <= 0); t = t.add(step)) {		//等同于 for(t+0; t<=2PI; t+=PI/40), +0是因为直接用t会报错
			...
			BigDecimal bx = new BigDecimal((ApfloatMath.sin(aT)).toString());		//真正的desX = sin(t), 计算之后转回BigDecimal，写文件时Apfloat会使用科学计数法（其实mc能读，debug方便）
			BigDecimal by = new BigDecimal((ApfloatMath.cos(aT)).toString());		//真正的desY = cos(t), 同样转回BigDecimal
			String name = String.format("endRod_Circle_%.2f_%.2f", destinationX.doubleValue(), destinationY.doubleValue());
																					//文件名称：endRod_Circle_desX（两位小数）_desY（两位小数）
			...
			String identifier = String.format("\"identifier\": \"minecraft:endRod_circle_%.2f_%.2f\",", destinationX.doubleValue(), destinationY.doubleValue());
	...
	public static void main(String[] args) throws IOException {
		CustomParticle_Circle.Circle(); //调用方法，生成80种不同的粒子
	}
}
