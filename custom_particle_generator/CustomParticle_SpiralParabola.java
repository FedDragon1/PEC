...
public class CustomParticle_SpiralParabola {
	public static void Horizontal () throws IOException {	//这里之所以不需要粒子名称是因为如果需要不同的粒子，base_uv里的值也会相应更改，而需要换行的粒子则更加复杂（第二种粒子）
		...
		for (t.add(Zero); (t.compareTo(Tau) <= 0); t = t.add(step)) {		//等同于 for(t+0; t<=2PI; t+=PI/40), +0是因为直接用t会报错
			...
			String name = String.format("endRod_Horizontal_%.2f_%.2f", destinationX.doubleValue(), destinationY.doubleValue());
																					//文件名称：endRod_Horizontal_desX（两位小数）_desY（两位小数）
			...
			String identifier = String.format("\"identifier\": \"minecraft:endRod_horizontal_%.2f_%.2f\",", destinationX.doubleValue(), destinationY.doubleValue());
			...
			output.write("\"" + bx + " * Math.pow(variable.particle_age, 0.25)\"," + '\n' + "          ");
																			//以 √x的速率，将粒子移动到正确的x轴坐标
	...
	public static void main(String[] args) throws IOException {
		CustomParticle_SpiralParabola.Horizontal();
	}
}
