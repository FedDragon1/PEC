...
public class CustomParticle_Star {
	public static void Star () throws IOException {	//这里之所以不需要粒子名称是因为如果需要不同的粒子，base_uv里的值也会相应更改，而需要换行的粒子则更加复杂（第二种粒子）
		...
		for (t.add(Zero); (t.compareTo(Tau) <= 0); t = t.add(step)) {		//等同于 for(t+0; t<=2PI; t+=PI/40), +0是因为直接用t会报错
			...
			BigDecimal bx = new BigDecimal((ApfloatMath.pow(ApfloatMath.sin(aT), 3)).toString());		//真正的desX = sin^3(t), 计算之后转回BigDecimal，写文件时Apfloat会使用科学计数法（其实mc能读，debug方便）
			BigDecimal by = new BigDecimal((ApfloatMath.pow(ApfloatMath.cos(aT), 3)).toString());		//真正的desY = cos^3(t), 同样转回BigDecimal
			String name = String.format("endRod_Star_%.2f_%.2f", destinationX.doubleValue(), destinationY.doubleValue());
																					//文件名称：endRod_Star_desX（两位小数）_desY（两位小数）
			...
			String identifier = String.format("\"identifier\": \"minecraft:endRod_star_%.2f_%.2f\",", destinationX.doubleValue(), destinationY.doubleValue());
			...
			if (t.compareTo(Tau.divide(Four)) <= 0) {		//如果是前四分之一（圆的右上角）；等同于 if (t <= PI / 2)
				destinationX = destinationX.add(OneOverTwenty);			//desX += 0.05
				...
				destinationX = destinationX.subtract(OneOverTwenty);	//desX -= 0.05
				destinationY = destinationY.subtract(OneOverTwenty);	//desY -= 0.05
